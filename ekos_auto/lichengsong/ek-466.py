...
appname_tmp = "stress-changePodNum-"
app_num = 5
...
#changePodNum
PodNum = [3,5,1]
obj_json = {"name":"test","namespace":"default","replicas":"None"}
url_ChangePod = "http://" + ip + ":30000/service/stack/api/app/scale" 
for i in range(len(PodNum)):
	obj_json["replicas"] = PodNum[i]
	pod_rtn = my_utils.call_rest_api(url_ChangePod,"POST",json=json.dumps(obj_json))
	if "success" in json.loads(pod_rtn)["status"]:
		info('Change replicas successfully,now is: %s' % obj_json["replicas"]) 
	else:
		sys.exit()
