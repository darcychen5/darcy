...
class Changeip(unittest.TestCase):
    ...
    def test_changeip(self):
        ...
        new_dns = '8.8.8.8'
        rtn = self.tests.tools.change_usx_ip(new_ip=new_ip, netmask=new_netmask, gateway='10.16.0.1', dns=new_dns)
        if self.utils.is_poweron(new_ip):
            ...
            print('get message from resolv.conf is %s' % result)
            if new_dns not in result:
                print("the DNS not store in /etc/resolv.conf")
                return False

            # check /etc/resolv.conf after reset amc
